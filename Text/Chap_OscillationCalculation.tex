...
Where \quickmath{L(i,j,k,l)} represents the sum of the of the path lengths with fixed zenith angle and production heights corresponding to the \quickmath{i^{th}}, \quickmath{j^{th}}, \quickmath{k^{th}} and \quickmath{l^{th}} percentile of the cuumulative distribution function. The values that are taken as \quickmath{\beta} (and values for \quickmath{L_{\beta}}) are chosen to smooth the oscillation contours in \quickmath{\Delta m^{2}_{32}} without incurring loss of sensitivity \cite{t2k_tn_425}.
