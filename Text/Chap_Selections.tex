...
\ChangeRM{The single-ring and multi-ring PID systematics encode the detector's ability to separate electron-like and muon-like events and are implemented in a similar way to the ring separation systematic.}
\Change{Two electron-muon separation systematics are included within this model which anti-correlate the response of the electron-like and muon-like samples: one for single-ring events and another for multi-ring events.}

The multi-ring electron-like separation likelihood, discussed in \autoref{sec:SelsAndSysts_Sels_Atms}, encodes the ability of the detector to separate neutrino from anti-neutrino events.
\ChangeRM{As an important systematic in the mass hierarchy determination, systematic uncertainties control the relative normalisations of the $\nu_e$ and $\bar{\nu}_e$ enriched samples. Two normalisation parameters are implemented which vary the event rate of each multi-ring sample, whilst ensuring the total event rate is conserved.}
\Change{Two normalisation parameters vary the relative normalisation of multi-ring \quickmath{\nu_e} and \quickmath{\bar{\nu}_e} samples whilst keeping a consisntent overall event rate.}
