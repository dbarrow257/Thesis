...
void ApplyReacPrior(TH2D* Hist) {

  for (int xBin=1;xBin<=Hist->GetNbinsX();xBin++) {
    double STH13 = Hist->GetXaxis()->GetBinCenter(xBin);

    double Chi2 = (STH13 - 0.0212)/0.0008;
    double Prior = std::exp(-0.5 * Chi2 * Chi2);

    std::cout << STH13 << " " << Chi2 << " " << Prior << std::endl;

    for (int yBin=1;yBin<=Hist->GetNbinsY();yBin++) {
      Hist->SetBinContent(xBin,yBin,Hist->GetBinContent(xBin,yBin)*Prior);
    }
  }

}

void Draw_withReacPrior() {
  ...
  for (int iHist=0;iHist<nSummedHists;iHist++) {
    ApplyReacPrior(SummedHists_App[iHist]);
  }  

  for (int iHist=0;iHist<nSummedHists;iHist++) {
    SummedHists_App[iHist]->GetXaxis()->SetRangeUser(0.0212-2*0.0008,0.0212+2*0.0008);
  }
  
  TString AppOutputName = "AppearanceScans_withReacPrior.pdf";
