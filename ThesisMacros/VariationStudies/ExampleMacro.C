...
void plotVariableGraphs(std::vector< std::vector<TGraph*> > VariableGraphs, std::vector< std::vector<double> > MeanVector, std::vector<TString> TreeNames, TString OutputName, double XAxisRange, TString XAxisCaption, TString YAxisCaption) {
  ...
  for (int iTree=0;iTree<nTrees;iTree++) {
    ...
    for (int iPlot=0;iPlot<nPlots;iPlot++) {
      ...
      VariableGraphs[iTree][iPlot]->GetXaxis()->SetLabelSize(0.055);
      VariableGraphs[iTree][iPlot]->GetXaxis()->SetTitleOffset(1.2);
      
      VariableGraphs[iTree][iPlot]->GetXaxis()->SetTitle(XAxisCaption);
      VariableGraphs[iTree][iPlot]->GetYaxis()->SetTitle(YAxisCaption);

      //TLine* Line = new TLine(MeanVector[iTree][iPlot],0.,MeanVector[iTree][iPlot],YAxisRange);
...
void plotSquaredDifferenceGraphs(std::vector< std::vector<TGraph*> > SquaredDifferenceGraphs, std::vector<TString> TreeNames, TString OutputName, double XAxisRange, TString XAxisCaption, TString YAxisCaption) {
  ...
  for (int iTree=0;iTree<nTrees;iTree++) {
    ...
    for (int iPlot=0;iPlot<nPlots;iPlot++) {
      ...
      SquaredDifferenceGraphs[iTree][iPlot]->GetYaxis()->SetRangeUser(0.,XAxisRange);
      //SquaredDifferenceGraphs[iTree][iPlot]->GetXaxis()->SetLimits(0.,0.1);

      SquaredDifferenceGraphs[iTree][iPlot]->GetXaxis()->SetLabelOffset(0.02);
      SquaredDifferenceGraphs[iTree][iPlot]->GetXaxis()->SetLabelSize(0.05);
      SquaredDifferenceGraphs[iTree][iPlot]->GetXaxis()->SetTitleOffset(1.2);
...
void plotVarianceGraphs(std::vector<TGraph*> VarianceGraphs, std::vector<TString> TreeNames, TString OutputName, double XAxisRange, TString XAxisCaption, TString YAxisCaption) {
  ...
  for (int iTree=0;iTree<nTrees;iTree++) {
    ...
    //VarianceGraphs[iTree]->GetXaxis()->SetLimits(0.,0.1);

    VarianceGraphs[iTree]->GetXaxis()->SetLabelOffset(0.02);
    VarianceGraphs[iTree]->GetXaxis()->SetLabelSize(0.05);
    VarianceGraphs[iTree]->GetXaxis()->SetTitleOffset(1.2);
...
void ExampleMacro() {
  ...
  Variable_Graphs[Sample][Throw]->GetYaxis()->SetTitleSize(0.05);
  Variable_Graphs[Sample][Throw]->GetXaxis()->SetTitleOffset(0.9);
  Variable_Graphs[Sample][Throw]->GetYaxis()->SetTitleOffset(0.9);
  Variable_Graphs[Sample][Throw]->Draw();
  ...
  for (int iPoint=0;iPoint<Variable_Graphs[Sample][Throw]->GetN();iPoint++) {
    Variable_Graphs[Sample][Throw]->GetPoint(iPoint,X,Y);
    if (X == SubDivisionBinning[PointToDraw]) {
      break;
    }
  }
  ...
  int nPointsToDraw = 5;
  std::vector<int> PointsToDraw(nPointsToDraw);
  PointsToDraw[0] = 1;
  PointsToDraw[1] = 2;
  PointsToDraw[2] = 3;
  PointsToDraw[3] = 4;
  ...
  std::vector<int> PointsToDraw_Color(nPointsToDraw);
  PointsToDraw_Color[0] = kBlack;
  PointsToDraw_Color[1] = kRed;
  PointsToDraw_Color[2] = kBlue;
  PointsToDraw_Color[3] = kCyan;
  PointsToDraw_Color[4] = kGreen;
  
  std::vector< std::vector<double> > DifferenceDistribution(nPointsToDraw);
  for (int iPoint=0;iPoint<nPointsToDraw;iPoint++) {
    DifferenceDistribution[iPoint].resize(nOscThrows);
  }
  for (int iThrow=0;iThrow<nOscThrows;iThrow++) {
    for (int iPoint=0;iPoint<nPointsToDraw;iPoint++) {
      std::cout << "Point;" << iPoint << " " << Variable_Vector[Sample][iThrow][PointsToDraw[iPoint]] << " "  << Mean_Vector[Sample][Throw] << std::endl;
      DifferenceDistribution[iPoint][iThrow] = fabs(Variable_Vector[Sample][iThrow][PointsToDraw[iPoint]]-Mean_Vector[Sample][iThrow]);
    }
  }

  for (int iThrow=0;iThrow<nOscThrows;iThrow++) {
    for (int iPoint=0;iPoint<nPointsToDraw;iPoint++) {
      if (DifferenceDistribution[iPoint][iThrow] > Max) Max = DifferenceDistribution[iPoint][iThrow];
      if (DifferenceDistribution[iPoint][iThrow] < Min) Min = DifferenceDistribution[iPoint][iThrow];
    }
  }

  std::cout << "Found Min/Max:" << Min << "/" << Max << std::endl;
  
  std::vector<TH1D*> DiffHist(nPointsToDraw);
  for (int iPoint=0;iPoint<nPointsToDraw;iPoint++) {
    DiffHist[iPoint] = new TH1D(Form("DifferenceDistribution_%i",iPoint),"",40,Min,Max/6);
    DiffHist[iPoint]->SetLineColor(PointsToDraw_Color[iPoint]);
    DiffHist[iPoint]->SetLineWidth(3);
  }

  for (int iPoint=0;iPoint<nPointsToDraw;iPoint++) {
    for (int iThrow=0;iThrow<nOscThrows;iThrow++) {
      DiffHist[iPoint]->Fill(DifferenceDistribution[iPoint][iThrow]);
    }
  }

  DiffHist[nPointsToDraw-1]->GetXaxis()->SetTitle("|#lambda^{N} - #bar{#lambda}|");
  DiffHist[nPointsToDraw-1]->GetYaxis()->SetTitle("Number of Throws");
  DiffHist[nPointsToDraw-1]->GetXaxis()->SetTitleSize(0.06);
  DiffHist[nPointsToDraw-1]->GetYaxis()->SetTitleSize(0.05);
  DiffHist[nPointsToDraw-1]->GetXaxis()->SetTitleOffset(0.9);
  DiffHist[nPointsToDraw-1]->GetYaxis()->SetTitleOffset(0.9);
  
  gStyle->SetOptStat(false);
  
  TCanvas* DiffCanv = new TCanvas("DiffCanv","");
  DiffCanv->SetBottomMargin(0.15);
  for (int iPoint=(nPointsToDraw-1);iPoint>=0;iPoint--) {
    if (iPoint==(nPointsToDraw-1)) {
      DiffHist[iPoint]->Draw();
    } else {
      DiffHist[iPoint]->Draw("SAME");
    }
  }
  ...
  for (int iPoint=(nPointsToDraw-1);iPoint>=0;iPoint--) {
    Legend->AddEntry(DiffHist[iPoint],Form("N = %i",PointsToDraw[iPoint]),"l");
  }
  Legend->Draw("SAME");

  DiffCanv->Print("DifferenceInEventRateToMean.pdf");
...
>  TString VariableGraphOutputName = BranchName+"_VariableGraphs.pdf";
  ...
  plotVariableGraphs(Variable_Graphs,Mean_Vector,TreeNames,VariableGraphOutputName,SubDivisionBinning[nSubDivisionBinning-1],"SubDivision","Event Rate");
  ...
  plotVarianceGraphs(Variance_Graph,TreeNames,VarianceGraphOutputName,SubDivisionBinning[nSubDivisionBinning-1],"SubDivision","Variance in Event Rate");
  */
}
