...
void Draw(TString SampleName, bool fixedAdler) {
  gStyle->SetOptStat(false);

  if (fixedAdler) {
    std::cout << "Making predictives with fixed Adler Angle Dial";
  } else {
    std::cout << "Making predictives with varied Adler Angle Dial";
  }
  ...
  TString PreFitPredFileName;
  if (fixedAdler) {
    PreFitPredFileName = "../../../MacroInputs/DowngoingPredictives/EffectOfAdlerAngle/MaCh3-Atmospherics-MCMC_PreFitFixedAdlerAngle_Pred.root";
  } else {
    PreFitPredFileName = "../../../MacroInputs/DowngoingPredictives/EffectOfAdlerAngle/MaCh3-Atmospherics-MCMC_PreFitVariedAdlerAngle_Pred.root";
  }
  TFile* PreFitPredFile = new TFile(PreFitPredFileName);

  TString PostFitPredFileName;
  if (fixedAdler) {
    PostFitPredFileName = "../../../MacroInputs/DowngoingPredictives/EffectOfAdlerAngle/MaCh3-Atmospherics-MCMC_PostFitFixedAdlerAngle_Pred.root";
  } else {
    PostFitPredFileName = "../../../MacroInputs/DowngoingPredictives/EffectOfAdlerAngle/MaCh3-Atmospherics-MCMC_PostFitVariedAdlerAngle_Pred.root";
  }
  ...
  TString CanvName = "Predictive_"+SampleName;
  if (fixedAdler) {
    CanvName += "_FixedAdler";
  } else {
    CanvName += "_VariedAdler";
  }
  CanvName += ".pdf";  
