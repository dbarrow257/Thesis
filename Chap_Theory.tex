...
For an experiment observing atmospheric neutrinos propagating through the Earth, such as the studies presented in this thesis, a model of the Earth's density and layering is required. The model used within this analysis is the Preliminary Reference Earth Model (PREM) \cite{Dziewonski1981-sp}. This model provides piecewise cubic polynomials. To follow the methodology set in \cite{thesis_roger}, this analysis simply approximates the model as four layers of constant density with values taken from the PREM model, as described in \autoref{tab:NeutrinoOscillationPhysics_PREMModel}. As seen in \finish{Link to Oscillation chapter}, the approximation in this methodology has been removed and the density is instead integrated along the track using the piecewise polynomials.
